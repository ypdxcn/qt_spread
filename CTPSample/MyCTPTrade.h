...
class  CTPSAMPLE_EXPORT  MyCTPTrade
{
	...
	public:
		...
		///客户端认证响应
		virtual void OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) { m_Parent->OnRspAuthenticate(pRspAuthenticateField,pRspInfo, nRequestID, bIsLast); }
...
public:
	...
	atomic<bool> m_bQryIns;// 是否正在查询合约，如果正在查询，行情服务就不自己主动去订阅行情，因为查询合约完成后，会有订阅行情的动作
	atomic<bool> m_bQryInsOK;// 是否查询合约成功，成功订阅行情的动作
