...
string bytesToHex(const unsigned char *input, size_t length) 
{
    string str;
    str.reserve(length << 1);
    for (size_t i = 0; i < length; ++i) 
...
string MD5::ToMD5(const char *pBuf, unsigned int bufLen) 
{
    MD5_CTX context;
    ...
    MD5Init (&context);
    MD5Update (&context, (unsigned char *)pBuf, bufLen);
    MD5Final (digest, &context);
