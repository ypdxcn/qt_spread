...
std::string CTradePacket::AssembleReqHead(HEADER_REQ& v)
{
	...
	sprintf(tempcode,"%s%ds","%-",SEQNO);
	sprintf(code,tempcode,v.seq_no);  
	sEncode+=code;
	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",MSGTYPE);
	sprintf(code,tempcode,v.msg_type);
	sEncode+=code;
	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",EXCHCODE);
	sprintf(code,tempcode,v.exch_code);
	sEncode+=code;
	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",MSGFLAG);
	sprintf(code,tempcode,v.msg_flag);
	sEncode+=code;
	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",TERMTYPE);
	sprintf(code,tempcode,v.term_type);
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",USERTYPE);
	sprintf(code,tempcode,v.user_type);
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",USERID);
	sprintf(code,tempcode,v.user_id); 
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",AERACODE);
	sprintf(code,tempcode,v.area_code); 
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",BRANCHID);
	sprintf(code,tempcode,v.branch_id); 
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",CTELLERID1);
	sprintf(code,tempcode,v.c_teller_id1); 
	sEncode+=code;

	memset(tempcode,0x00,128);
	memset(code,0x00,1024);
	sprintf(tempcode,"%s%ds","%-",CTELLERID2);
...
void CTradePacket::Decode(const char * pData, unsigned int nLength)
{
	...
	if(m_HeadMsg.m_type == HEAD_REQ)
	{
        ...
        // modified by Jerry Lee, 2011-4-27, 解决pcBuf内容中含有空字符时sBody不正确的问题
	    //sBody = pcBuf;
        sBody.append(pcBuf, nLength - nHeadLen);
	}
...
int CTradePacket::GetRspCode( string &sCode )
{
	sCode = m_HeadMsg.uheader.Rsp_Hmsg.rsp_code;
