...
void frmLoginDialog::initStyle()
{
		...
		ui->horizontalLayout_main->setGeometry(QRect(0, 0, 471, 395));// width(), height()));


        ui->label_login_png->setPixmap(QPixmap::fromImage(QImage(":/res/login")));
        ui->label_login_png->setFixedWidth(395);//389
        ui->label_login_png->setFixedHeight(598);//598
        QStringList list;
...
//void frmLoginDialog::SetLoginMessage(const QString & str)
//{
//	ui->label_note->setText(str);
//};

void   frmLoginDialog::setLoginState(int val)
{
	loginRet = val;
}
