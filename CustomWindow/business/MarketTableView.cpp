...
MarketTableView::MarketTableView(QSplitter *spliter,QAbstractItemModel *model, QWidget *parent)
    :QTableView(parent)
{
    ...
    connect(updateTimer, &QTimer::timeout, this, &MarketTableView::responseTimer);
    connect(m_interval, &QTimer::timeout, this,  &MarketTableView::ClickedInterval);
...
void MarketTableView::setMainWinId(QWidget * id)
{
	m_mainWndid = id;
}
...
void MarketTableView::init()
{
    ...
    connect(this, &QAbstractItemView::clicked, this, &MarketTableView::RowClickSelected);
...
void MarketTableView::showMarketChangeDetails(const QModelIndex &current, const QModelIndex &previous)
{
	RowClickSelected(current);
}

void  MarketTableView::InitColumn()
{
	// 读取用户选择列的位置，保存到内存
	...
	QString  qsPath = g_Global.GetListIniPath("Quotation");
	QString buf;
...
void  MarketTableView::InitRowData()
{
	...
	App::GetPriProfileString(g_Global.GetListIniPath("Quotation"), "Info", "AllColumnWidths", buf);
	...
	for (size_t i = 0; i < vecUserInsIndexs.size(); i++)
	{
		...
		const QUOTATION& qt = g_TraderCpMgr.m_QMapQuotation[CHJGlobalFun::qstr2str(inst)];
...
QString MarketTableView::GetQuotationValue(const QString &lpszInsId, const QUOTATION &quotation, const int rowIndex,const int &iPos)
{
	...
	{
	case 10:
		strValue = FormatPrice(quotation.m_uiLastSettle);  // 昨结算
		break;
	case 11:
		strValue = FormatPrice(quotation.m_uilastClose); // 昨收盘
		break;
	case 12:
		strValue = FormatPrice(quotation.m_uiOpenPrice); // 开盘价
		break;
	case 13:
		strValue = FormatPrice(quotation.m_uiHigh); // 最高价
		break;
	case 14:
		strValue = FormatPrice(quotation.m_uiLow); // 最低价
		break;
	case 3:
		strValue = FormatPrice(quotation.m_uiLast); // 最新价
		break;
	case 21:
		strValue = FormatPrice(quotation.m_uiClose); // 收盘价
		break;
	case 22:
		strValue = FormatPrice(quotation.m_uiSettle); // 结算价
		break;
	case 6:
	...
	case 7:
		...
		break;
	case 24:
		strValue = FormatPrice(quotation.m_Bid[1].m_uiPrice); // 买价2
		break;
	case 25:
		strValue = QString::number(quotation.m_Bid[1].m_uiVol); // 买量2
		break;
	case 26:
		strValue = FormatPrice(quotation.m_Bid[2].m_uiPrice); // 买价3
		break;
	case 27:
		strValue = QString::number(quotation.m_Bid[2].m_uiVol); // 买量3
		break;
	case 28:
		strValue = FormatPrice(quotation.m_Bid[3].m_uiPrice); // 买价4
		break;
	case 29:
		strValue = QString::number(quotation.m_Bid[3].m_uiVol); // 买量4
		break;
	case 30:
		strValue = FormatPrice(quotation.m_Bid[4].m_uiPrice); // 买价5
		break;
	case 31:
		strValue = QString::number(quotation.m_Bid[4].m_uiVol); // 买量5
		break;
	case 8:
	...
	case 9:
		...
		break;
	case 32:
		strValue = FormatPrice(quotation.m_Ask[1].m_uiPrice); // 卖价2
		break;
	case 33:
		strValue = QString::number(quotation.m_Ask[1].m_uiVol); // 卖量2
		break;
	case 34:
		strValue = FormatPrice(quotation.m_Ask[2].m_uiPrice); // 卖价3
		break;
	case 35:
		strValue = QString::number(quotation.m_Ask[2].m_uiVol); // 卖量3
		break;
	case 36:
		strValue = FormatPrice(quotation.m_Ask[3].m_uiPrice); // 卖价4
		break;
	case 37:
		strValue = QString::number(quotation.m_Ask[3].m_uiVol); // 卖量4
		break;
	case 38:
		strValue = FormatPrice(quotation.m_Ask[4].m_uiPrice); // 卖价5
		break;
	case 39:
		strValue = QString::number(quotation.m_Ask[4].m_uiVol); // 卖量5
		break;

	case 15:
		strValue = QString::number(quotation.m_uiVolume);  // 成交量
		break;
	case 16:
		strValue = FormatPrice(quotation.m_uiWeight); // 成交（双边）重量
		break;
	case 19:
		strValue = FormatPrice(quotation.m_uiHighLimit); // 涨停板
		break;
	case 20:
		strValue = FormatPrice(quotation.m_uiLowLimit); // 跌停板
		break;
	case 40:
		strValue = QString::number(quotation.m_uiChiCangLiang);  // 持仓量
		break;
	case 4:
		//strValue = FormatPrice(quotation.m_uiUpDown); // 涨跌
		strValue = CHJGlobalFun::FormatFloat(QString::number(quotation.m_uiUpDown / g_Global.m_dQTFactor));
		break;
	case 5:
		strValue = QString::number(quotation.m_dUpDownRate , 'f',2);
		//strValue =  CHJGlobalFun::FormatFloat(CHJGlobalFun::DoubleToString(quotation.m_uiUpDownRate / 100000.00)).c_str(); // 涨跌幅度
		break;
	case 17:
		//strValue = FormatPrice(quotation.m_uiTurnOver); // 成交额
		strValue = CHJGlobalFun::FormatFloat(QString::number(quotation.m_uiTurnOver / g_Global.m_dQTFactor,'f',2));
		break;
	case 18:
		strValue = FormatPrice(quotation.m_uiAverage); // 均价
		break;
		/*
		case 43:
		strValue.Format("%d", quotation.m_uiDate);
		//strValue = CGlobalFun::FormatDate(quotation.quoteDate.c_str()); // 行情日期
		break;
		*/
	case 23:
		...
		break;
	case 41:
		strValue = QString::number( quotation.m_uiSeqNo);
		break;
	case 42:
		strValue = g_TraderCpMgr.GetProdCodeName(lpszInsId) ;
		break;
	case 2:
		strValue = g_TraderCpMgr.GetInsStateNameFromID(g_TraderCpMgr.GetInsStateID(lpszInsId));
		break;
	case 0:
...
void MarketTableView::SelMenu(QAction *pAction)
{
...
void  MarketTableView::RowClickSelected(const QModelIndex &index)
{
...
void MarketTableView::HeaderSectionMoved(int /*l*/, int o, int n)
{
    if (n == 32)
    {
...
void MarketTableView::HeaderActived(const QModelIndex &/*index*/)
{
    emit   StateChanged(true);
}

void MarketTableView::OnMenu(QAction *pAction)
{
...
void MarketTableView::GetHeadNames()
{
...
void MarketTableView::SetTableColor()
{
    if (model() && this)
    {
...
void MarketTableView::LastSortIndex(int index, Qt::SortOrder order)
{
...
void MarketTableView::responseTimer()
{
	//还原所有背景色
	//6列，8列，10到15列
	QuoteStandardItemModel* p = (QuoteStandardItemModel *)m_pmodel;
...
void MarketTableView::ClickedInterval()
{
...
void MarketTableView::onColorChange()
{
...
void MarketTableView::CloseTimer()
{
    if (updateTimer->isActive())
        updateTimer->stop();
}

void MarketTableView::StartTimer()
{
    if (!updateTimer->isActive())
        updateTimer->start();
}

void MarketTableView::focusInEvent(QFocusEvent *event)
{
}

void MarketTableView::focusOutEvent(QFocusEvent *event)
{
    //	CloseTimer();
}

void MarketTableView::mousePressEvent(QMouseEvent *event)
{
...
//void MarketTableView::SetMarket(std::vector<TSecurity*> *pVecSecurity)
//{
//    m_pVecSecurity = pVecSecurity;
//}

void MarketTableView::SortMarketData(std::vector<TableRow> &vecData, int nCol, SORT_DIRECTOR dir)
{
    if (nCol > 31)
        return;
...
void MarketTableView::SwapRow(std::vector<TableRow> &vecData, int nSrc, int nDest)
{
    std::swap(vecData[nSrc], vecData[nDest]);
}

void MarketTableView::ClearData()
{
    m_bResetData = true;
    m_bResetSel = true;
...
//垂直滑条变动
void MarketTableView::verticalScrollbarValueChanged(int value)
{
...
//水平滑条变动
void  MarketTableView::horizontalScrollbarValueChanged(int value)
{
    m_nHSrcollValue = 32 - value;
	m_nResize = 5;
}

void MarketTableView::horizontalScrollbarAction(int action)
{
    qDebug()<<action;
}


void MarketTableView::setModelColor(const QColor &up, const QColor &down, const QColor &none,const QColor &normal)
{
    QuoteStandardItemModel *model = qobject_cast<QuoteStandardItemModel *>(m_pmodel);
    model->setUpDownColor(up,down,none,normal);
}

void MarketTableView::setColorFromTheme()
{
    //LxSysThemeMgr::ColorScheme cs = LxSysThemeMgr::GetInstance()->getCurrentUseColorScheme();
    /*QColor up = LxSysThemeMgr::findColor(cs.Market,_MY_TEXT("列表报价上涨"));
    QColor down = LxSysThemeMgr::findColor(cs.Market,_MY_TEXT("列表报价下跌"));
...
//双击行
void MarketTableView::slotRowDoubleClicked(const QModelIndex& index)
{
	//通过Model获取一行  
